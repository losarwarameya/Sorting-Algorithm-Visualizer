...
def draw(draw_info, algo_name, ascending):
    ...
    sorting = draw_info.FONT.render(
        "Algorithm type : I - Insertion Sort | B - Bubble Sort", 1, draw_info.BLACK)
    draw_info.window.blit(
...
def draw_list(draw_info, color_positions={}, clear_bg=False):
    ...
    for i, val in enumerate(lst):
        ...
        number = draw_info.NUMBER_FONT.render(str(lst[i]), 1, draw_info.BLACK)
        draw_info.window.blit(
            number, (x + (draw_info.block_width/2 - number.get_width()/2), y-25))
